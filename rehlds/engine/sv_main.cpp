...
void SV_WriteSpawn(sizebuf_t *msg)
{
	...
	else
	{
		...
		// memory region from pr_strings (multi-TB apart on amd64/arm64).
		// ptrdiff cast to string_t (uint32) truncates to garbage. Allocate
		// in the engine StrPool instead — bounded offset, fits cleanly.
		sv_player->v.netname = AllocEngineString(host_client->name);
#else
		// 32-bit: the entire address space fits in uint32 so the original
		// ptrdiff round-trips correctly. Keep upstream's exact byte-output
		// here so the testdemos suite's strict packet comparisons still
		// match the recorded reference.
...
#if INTPTR_MAX > INT32_MAX
	// 64-bit: anchor pr_strings to the start of the Ed_StrPool buffer so
	// every string allocated via Ed_StrPool_Alloc (which is what
	// ED_NewString uses under REHLDS_FIXES) lies at an offset >= 0 from
	// pr_strings, bounded by the pool's 128 KB capacity. The original
...
#else
	// 32-bit: keep upstream's gNullString anchor — int fits any ptrdiff
	// in a 4 GB address space, so the legacy AllocEngineString computation
	// round-trips correctly. Maintains byte-for-byte parity with the
	// recorded testdemos suite for the upstream PR.
...
#else
	// 32-bit: keep the upstream pointer-as-offset hack so the recorded
	// testdemos packets see byte-identical values for pev->model.
...
#else
	// 32-bit: byte-parity with upstream for testdemos.
