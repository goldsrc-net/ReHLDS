...
// MSVC inline `__asm { mov eax, [ebp + 4] }` reads the caller's return
// address off the stack frame; that only works in 32-bit mode (ebp/esp
// don't exist on x64, where the same probe would need rbp/rsp anyway,
// and MSVC dropped __asm support entirely on x64). Gate to x86; treat
// as a no-op everywhere else, mirroring the GCC __i386__ branch below.
